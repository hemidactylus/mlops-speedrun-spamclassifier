...
from api.model_serving.models.response import FeaturesResult, PredictionResult
...
def _format_prediction(pred_dict, input, echo_input):
        result = {
            **{
...
def createModelRouter(version, model_wrapper):
    ...
    @modelRouter.post('/features_to_prediction', response_model=PredictionResult, tags=[version])
    async def serve_features_to_prediction(params: FeatureInput):
        ...
        return _format_prediction(prediction_dict, params.text, params.echo_input)

    @modelRouter.post('/text_to_prediction', response_model=PredictionResult, tags=[version])
